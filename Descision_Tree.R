...
tab <- table(predict(tree),train$Survived)
print(tab)
...
train <- data[pd==1,]
validate <- data[pd==2,]
mylogit <- glm(Survivedf~MaternalAge+GestationalAge+GeneticAbnorm+Congeniticalabnorm+
                 Rhdisease+Placental.UmbillicalcordAccident,family="binomial",data=train)
summary(mylogit)

mylogit <- glm(Survivedf~MaternalAge+GestationalAge+GeneticAbnorm+Congeniticalabnorm+
                 Rhdisease,family="binomial",data=train)
summary(mylogit)

#logistic regression
pred <- predict(mylogit,validate,type="response")
#missclassification
pred
head(validate)
pred[2]
...
model<-rep("0",406)
model[pred>0.5]<-"1"
tab<-table(model,validate$Survivedf)
print(tab)
1-sum(diag(tab))/sum(tab)
